...
import { publications } from '../constants/index';
  ...
  <motion.div variants={fadeIn('up', 'spring', index * 0.5, 1) as Variants}>
    ...
    >
      <div
        ...
        tabIndex={0}
        onClick={() => paperLink && window.open(paperLink, '_blank', 'noopener,noreferrer')}
        onKeyDown={(event) => {
          if (event.key === 'Enter' && paperLink) {
      ...
      <div className='px-2 py-5 text-center lg:px-5 lg:py-2 xl:px-10 xl:py-8'>
        <h3 className='text-[24px] font-bold text-text'>{title}</h3>
        <p className='mt-1 text-[12px] text-text opacity-75'>{venue}</p>
        <p className='mt-2 text-[17px] text-text opacity-75'>{publication}</p>
  ...
  return (
    <>
      ...
      <div className='flex w-full'>
        ...
        >
          Selected research and publications that highlight my work at the intersection of machine learning, knowledge graphs, and retrieval-augmented
          generation.
        </motion.p>
      </div>
