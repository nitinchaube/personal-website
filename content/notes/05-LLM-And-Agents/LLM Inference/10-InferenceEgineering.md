---
title: "All about Inference Engineering"
date: 2026-06-22
summary: "An end-to-end guide to LLM inference, from the basics up: what a forward pass does, GPU architecture and the memory budget, the prefill/decode split, the KV cache, continuous batching, FlashAttention and PagedAttention, quantization, speculative decoding, multi-GPU sharding, prefill-decode disaggregation, choosing a serving stack, benchmarking, and the MLOps around all of it."
...
Before any of the tricks make sense, it helps to be clear on what inference actually is. Inference is just the model running forward: you hand it some text, it turns that text into tokens (the little sub-word chunks it actually reads), runs those tokens through every layer once, and spits out a probability distribution over what the next token should be. You sample one token from that distribution, glue it onto the end of the input, and run the whole thing again. That loop is the entire game. Training is where the weights get learned and it happens rarely, inference is where those frozen weights get used and it happens constantly. Nothing about the weights changes at inference time, so every knob you have is about *how* you run that forward pass, not *what* the model knows.

The concepts that keep coming up are all downstream of that loop. There's the prompt you send in and the tokens the model streams back out. There's the KV cache, which is the model remembering the work it already did for earlier tokens so it doesn't redo it every step. There's batching, which is running many requests through the same weights at once so you're not paying to load a 70B model just to serve one person. And there's the constant tension between latency (how fast one user gets their answer) and throughput (how many users you can serve per GPU per second). Almost everything below is some clever way of managing memory, reusing computation, or packing more work into a single pass, so it's worth holding those four ideas (tokens, the cache, batching, latency vs throughput) in your head as you read.
...
Before optimizing anything, it helps to have a concrete picture of what the model does when a request comes in. Strip away the serving machinery and a decoder-only transformer (GPT, Llama, Qwen, Mistral, DeepSeek, all the ones you serve) does the same five things every time.

1. **Tokenize.** The raw text gets chopped into tokens, the sub-word chunks the model actually reads, and each token is looked up as an integer ID in the model's vocabulary. "The quick brown fox" might become `["The", " quick", " brown", " fox"]` and then `[976, 4853, 19705, 68347]`. Every model ships its own tokenizer, so token counts differ between models for the same string.
2. **Embed.** Each token ID indexes into an embedding table and becomes a vector. Now you have a matrix of shape `(sequence length × hidden size)`, and from here on it's all floating-point math.
3. **Run the transformer stack.** Those vectors flow through every layer of the model. Each layer is two big pieces: a **self-attention** block where each token computes a Query, Key, and Value and mixes information across positions, and an **MLP** (feed-forward) block that transforms each position independently. Attention is where "the fox" learns it relates to "quick" and "brown," the MLP is where most of the parameters and most of the raw compute live.
4. **Project to logits.** After the last layer, one more matrix multiply against the vocabulary maps the final hidden vector to a score for *every* token in the vocabulary. That's the logits vector, one number per possible next token. A bigger vocabulary makes this last layer more expensive, because you score every candidate.
5. **Sample and repeat.** Turn those logits into probabilities, pick one token, append it to the sequence, and go back to step 3 with the sequence now one token longer. Keep going until you hit a stop token, a length limit, or a stop string. At the very end, detokenize the generated IDs back into text.

```
"The fox" ─▶ [tokenize] ─▶ [976, 4853] ─▶ [embed] ─▶ vectors
                                                        │
                    ┌───────────────────────────────────┘
                    ▼
          ┌──────────────────────┐   × N layers
          ...
          └──────────┬───────────┘              │
                     ▼                           │
              final hidden ─▶ [logits] ─▶ [sample] ─▶ "jumps"
                     └───────── append token, loop ──┘
```

Two facts from this loop drive everything else in this note. First, generation is **sequential**: token N+1 can't start until token N exists, so you can't just parallelize your way out of latency the way you would a normal service. Second, producing each token means reading the model's weights *and* the growing history out of GPU memory, which is why decode is memory-bound and why so much of the rest of this document is about not re-reading things you already computed.

A quick vocabulary note that keeps recurring: the **context window** is the maximum number of tokens (prompt plus generation) the model can attend to at once, and it's shared between what you send in and what comes out. The model has no real memory between calls, so in a multi-turn chat the whole conversation gets re-sent every turn and re-processed. That "re-sending" is exactly what prefix caching (later) exists to make cheap.

---

# The GPU, in just enough detail

Almost every performance claim in this note bottoms out in one hardware fact, so it's worth a short tour of what a GPU actually is. You don't need to write CUDA, but you do need a mental model of where data lives and how fast it moves.

**Compute is organized in SMs.** A data-center GPU is a big grid of **Streaming Multiprocessors** (an H100 has 132, an A100 has 108). Each SM runs thousands of lightweight threads, scheduled in groups of 32 called **warps**, all executing the same instruction on different data. Inside each SM sit the **tensor cores**, specialized units that do small matrix multiply-accumulates and deliver far more throughput than the general-purpose cores. Transformers are almost entirely matrix multiplies, so tensor cores are what you're really trying to keep fed. The catch is they only fire when your data is in the right precision and layout, which is half the reason kernel libraries exist.

**Memory is a pyramid, and it's the whole story.** Every level down is bigger but dramatically slower:

| Level | Size (H100) | Rough speed | Scope |
| ----- | ----------- | ----------- | ----- |
| Registers | 256 KB per SM | ~1 cycle | per thread |
| Shared memory / L1 | up to ~228 KB per SM | ~20 TB/s | per thread block |
| L2 cache | ~50 MB | ~12 TB/s | all SMs |
| HBM (VRAM) | 80 GB | ~3.35 TB/s | the whole GPU |

Registers and on-chip SRAM (shared memory / L1) are blindingly fast but tiny. **HBM** is the big pool where your weights, KV cache, and activations actually live, and it's an order of magnitude slower than on-chip memory, hundreds of cycles per access. This single gap, fast compute sitting next to slow-relative-to-compute HBM, is the reason "memory-bound" is a phrase that will haunt the rest of this document. A huge fraction of inference engineering is just moving data *up* this pyramid, keeping the bytes you reuse in SRAM instead of fetching them from HBM again and again. That's literally what FlashAttention does, and it's why decode (which streams the whole model from HBM per token) is so much harder to speed up than prefill.

Everything below is downstream of these two pictures: a sequential token loop, running on hardware where compute is cheap and moving bytes from HBM is expensive.
...
Which ceiling you hit depends on **arithmetic intensity**, the FLOPs performed per byte loaded.
...
# Sizing the box: what's actually in VRAM

The first practical question anyone asks is "what GPU do I need," and the honest answer is that you have to add up four things, not one. GPU memory holds:

1. **Model weights.** The fixed cost. Bytes = parameters × bytes-per-parameter. FP16/BF16 is 2 bytes, FP8/INT8 is 1 byte, INT4 is half a byte. So a 7B model in FP16 is ~14 GB, a 70B model is ~140 GB, and the same 70B at INT4 is ~35 GB. This is why quantization is often the difference between fitting on one card and needing four.
2. **KV cache.** The variable cost, and the one people forget. It grows with sequence length *and* with the number of concurrent requests, and on a busy server it routinely rivals or exceeds the weights. The whole KV-cache section later is really about controlling this number.
3. **Activations and workspace.** The scratch memory a forward pass needs, plus buffers the engine reserves for attention kernels, communication, and CUDA graphs. Smaller than the other two but not zero.
4. **Framework and CUDA overhead.** The CUDA context, the runtime's allocator, and so on, typically a couple of GB before you've served anything.

A quick sizing shortcut for the weights-plus-overhead baseline:

$$
\text{Memory (GB)} \approx P \times \frac{Q}{8} \times (1 + \text{overhead})
$$
...
Here's the conflict at the heart of serving. Batching is how you get throughput: you share the weight-loading cost across many sequences. But batching is also how you *hurt* latency:
...
2. To *form* a big batch you sometimes hold early requests in a queue waiting for friends to arrive, so **TTFT goes up**.
...
# Two regimes: online serving vs offline batch

Everything above assumes **online serving**: requests arrive whenever users show up, you can't see the future, and you're being graded on latency percentiles. That's the hard case and most of this note lives there. But a lot of real inference is the opposite regime, and it changes every tradeoff.

**Offline batch inference** is when you have a big pile of inputs and no user waiting: classifying millions of documents, generating embeddings, running an eval suite, producing synthetic training data, backfilling summaries. Nobody is watching a token stream, so latency stops mattering and you optimize purely for **throughput per dollar**. That flips your knobs the other way:

- Run the **biggest batches the memory allows**. There's no latency SLO to protect, so the usual "don't run hot" advice inverts, you *want* the GPU pinned near 100%.
- Sort or bucket inputs by length so a batch isn't dragged down by one outlier, and so padding waste stays low.
- Reach for cheaper hardware and heavier quantization than you'd dare use online, because a small quality hit on a bulk job is often fine and the cost savings are large.
- You can often use the engine in **library mode** (call `LLM.generate()` in-process) instead of standing up an HTTP server, since there's no external client to serve.

The mental split is simple: online serving trades throughput away to protect latency, offline batch spends latency freely to maximize throughput. Knowing which regime you're in tells you which half of this document to apply.

---

# Batching: where the throughput comes from

This is the single biggest source of throughput, and it has a clear evolution where each step fixes a specific waste in the one before it.
...
# FlashAttention: computing attention without thrashing memory
...
- **Greedy / temperature**: `temperature` flattens (>1) or sharpens (<1) the distribution, and 0 is deterministic argmax.
...
- **Graph compilers** like TensorRT-LLM and `torch.compile` do this fusion and layout selection automatically and emit hardware-tuned kernels. TensorRT-LLM tends to be the fastest on NVIDIA but the least flexible (you compile an engine per model/shape/precision), while `torch.compile` is more general and lives inside PyTorch.
...
None of the techniques above matter if you're calling `model.generate()` from raw Hugging Face Transformers in a loop. That's fine for a notebook, but it has no continuous batching, no PagedAttention, no proper scheduler, so it falls over the moment real concurrent traffic shows up. An **inference framework** is the layer that turns a model into a service: it owns batching, the KV cache, streaming, memory limits, and the request API. Picking the right one is most of the practical work.

### The serving runtimes worth knowing

- **vLLM**: the default starting point. PagedAttention originated here, it has continuous batching, prefix caching, speculative decoding, quantization, and wide model support, and it's easy to stand up an OpenAI-compatible server. If you don't have a specific reason to pick something else, start here.
- **SGLang**: very fast, strong on prefix-cache-heavy and structured-output workloads (its RadixAttention and frontend language are built for reuse across calls), and increasingly used for large distributed and disaggregated setups.
- **TensorRT-LLM**: usually the fastest on NVIDIA if you're willing to pay for it in flexibility. You compile a hardware-specific engine per model/shape/precision, which is great for a locked-down production model and painful when you're iterating.
- **Hugging Face TGI**: battle-tested and production-proven, though now in maintenance mode, so treat it as a known quantity rather than where the frontier is moving.
- **LMDeploy**: strong decode speeds and good quantization support, worth a look when memory is tight.
- **Edge and local**: `llama.cpp` (portable C/C++, runs anywhere, great on consumer GPUs), **MLC-LLM** (TVM-based, targets everything from phones to browsers), and **Ollama** (dead-simple local wrapper over llama.cpp, but single-user, no real concurrency). These are for laptops and edge devices, not for serving traffic.

One honest caveat: this list ages fast. A runtime that's fastest today may trail next quarter, and new models sometimes launch supported on only one engine. The pragmatic stance is to stay **runtime-agnostic** behind an OpenAI-compatible API so you can swap the engine without rewriting your app, and to actually benchmark the candidates on *your* workload rather than trusting anyone's headline number.

### Library mode vs server mode

Most of these run two ways. **Library mode** loads the engine in your own process (`LLM.generate()`), which is ideal for offline batch jobs and eval pipelines where you want direct access to outputs and no network hop, at the cost of the engine sharing your process's fate (a GPU OOM takes down everything). **Server mode** runs the engine as its own process behind an HTTP API, which is the right boundary when multiple apps share a model, clients speak different languages, or the serving layer needs to scale and deploy on its own. For anything user-facing, server mode is almost always what you want.

### Where you run it

- **Serverless / managed APIs** (OpenAI, Anthropic, and hosted open-model endpoints): zero infra, per-token pricing, someone else eats the GPU scaling problem. Best when you're moving fast, traffic is spiky, or you don't want to operate GPUs. You give up control over latency tuning, data residency, and unit economics at scale.
- **Self-hosted on your own GPUs**: maximum control over performance, cost, and privacy, and it's usually cheaper per token *at steady, high utilization*. You inherit the entire operational burden, autoscaling, cold starts, upgrades, on-call, that the rest of this note is about.
- **BYOC (bring your own cloud)** and **on-prem / air-gapped**: the middle and far ends of the control spectrum. BYOC runs a vendor's control plane inside your cloud account so data never leaves your boundary, on-prem is for strict compliance or when you already own the hardware. Both trade convenience for control and are driven mostly by data-governance and cost requirements, not by raw performance.

The usual arc: prototype against a managed API, move to self-hosted vLLM/SGLang once volume makes the per-token math hurt, and reach for a distributed inference platform once you're spanning many GPUs, regions, or clouds and need routing, failover, and autoscaling handled for you.
...
Every optimization in this note is a hypothesis until you measure it on your own workload, and measuring LLM inference is easy to do badly. "200 tokens/sec" means nothing on its own, because the number moves enormously with input length, output length, concurrency, framework version, and hardware. A result without that context isn't reproducible and isn't useful.

First, separate the two things people call "benchmarks." **Quality benchmarks** (MMLU, GSM8K, HumanEval) tell you whether the model is smart enough. **Performance benchmarks** tell you whether it's fast and cheap enough to serve, and that's the one this section is about. A model that tops a quality leaderboard can still be unshippable if its latency or cost don't fit, and the reverse happens too.

A few rules that keep benchmarks honest:

- **Measure at your target SLO, and report goodput, not raw throughput.** The question isn't "how many tokens/sec can this do," it's "how many tokens/sec while holding p99 TTFT and ITL under target." Push load up until you *just* violate the SLO, that point is your real capacity.
- **Fix and report input/output lengths.** A 256-in/512-out chat workload and a 8k-in/256-out RAG workload benchmark completely differently on the same stack. Use length distributions that look like your traffic, not a single convenient value.
- **Control concurrency and request rate explicitly**, and sweep them. One curve of latency-vs-concurrency tells you far more than a single point, because it shows you where the queue starts blowing up.
- **Pin the environment.** Framework and version, GPU type, parallelism config, quantization, and cache settings all change results, so record them alongside every number.
- **Mind warm vs cold caches.** With prefix caching on, a benchmark that reuses the same prompt is measuring your cache, not your engine. Decide which you mean to test.

For tooling: general load testers like **Locust** and **k6** are good for traffic shaping and finding infra bottlenecks, while purpose-built tools like **NVIDIA GenAI-Perf** and **LLMPerf** understand token-level metrics (TTFT, ITL, throughput) out of the box. Most engines also ship their own bench scripts (vLLM's and SGLang's are handy for quick checks), but watch their defaults, they're often tuned to look good rather than to match your production config. Whatever you use, a result is only worth quoting if someone else could reproduce it from what you wrote down:

| Category | Example |
| --- | --- |
| Model | `Llama-3.1-8B-Instruct`, FP16 |
| Framework | `vLLM 0.x`, TP=1 |
| Hardware | 1× H100 80GB |
| Workload | 256 in / 512 out, 32 concurrent, 50 req/s |
| Result | 210 ms median TTFT, 1.4 s p99 E2E, 98 tok/s, 89% GPU util, $/1M tokens |
...
- **GPU SM utilization** (DCGM exporter): read it *together* with queue depth. High SM + low queue = healthy compute-bound work. Low SM + high queue = a bottleneck *outside* the GPU (tokenization, the Python event loop, scheduling), so you're starving the GPU and adding GPUs won't help.
